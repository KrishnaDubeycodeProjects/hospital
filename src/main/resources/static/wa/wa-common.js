 ...
 */
(function (window) {
  // Universal fetch interceptor: inject ngrok-skip-browser-warning and authorization on all requests
  ...
  window.fetch = function (resource, init) {
    ...
    if (headers instanceof Headers) {
      ...
      const exists = headers.some(([k]) => k.toLowerCase() === 'ngrok-skip-browser-warning');
      if (!exists) {
        headers.push(['ngrok-skip-browser-warning', 'true']);
      }
    } else {
      headers = Object.assign({ 'ngrok-skip-browser-warning': 'true' }, headers || {});
    }
    opts.headers = headers;
    return _nativeFetch(resource, opts);
  };
